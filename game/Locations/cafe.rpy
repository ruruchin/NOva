label кафе_внутри:
    ...
    menu:
        "И ты действительно так обо мне беспокоилась?":
            call забота from _call_забота
    
        "Как будто твоя жизнь бы в момент развалилась":
            call сарказм from _call_сарказм
        
        "Что произошло пока меня не было?":
            call интерес from _call_интерес
...
label кафе_разговор:
    ...
    menu:
        "Рассказать про труп":
            call рассказ_про_труп from _call_рассказ_про_труп

        "Я просто проходил мимо":
            call просто_мимо from _call_просто_мимо
...
label кафе_продолжение:
    ...
    with Dissolve(1.2)
    hide black
    "Я пришел домой, день выдался довольно утомительным несмотря на то что я давно так не развеивался."
    "Мне было приятно повидаться со своими приятелями. Так что я сразу пошел в свою комнату чтобы набраться сил и поспать"
    scene black at zoom_1
    with Dissolve(1.2)
    hide bg room2
