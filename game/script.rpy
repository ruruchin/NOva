...
label paralax:
    ...
    gg"И может ли она быть полноценно искренней?" 
    ...
    gg"Внезапно я увидел небольшую улыбку на её лице, которая нарастала всё больше и больше."
    ...
    scene bg fonnav2 with Dissolve(0.5)
    ...
    gg"– В неожиданной для меня легкой манере ответила она мне"
    ...
    menu: 
        "Что-то типа паразита?":
            call parasit from _call_parasit
        "Ты довольна?":
            call blago from _call_blago 
...
label Hes_yshol:
    ...
    menu:
        "Подойти ближе":
            call label_podoyti_blyzhe from _call_label_podoyti_blyzhe
        
        "Не подходить":
            call label_ne_podhodit from _call_label_ne_podhodit
    return
...
label posle:
    ...
    "Это тоже оказало на меня большое влияние, иногда я видел, как она плакала, когда мне было плохо и я долго болел, она не отходила от меня ни на шаг и усердно трудилась лишь для того, чтобы я смог встать на ноги."
    ...
    menu:
        "Что она тебе успела наговорить?":
            call рассказать from _call_рассказать
            
        "Как у неё язык изо рта еще не выпал":
            call язвить from _call_язвить     
return
